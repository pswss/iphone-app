...
현재 프로젝트 설정의 마케팅 버전은 `1.1.1`, 로컬 빌드 번호는 `1`입니다. Xcode Cloud는 자체 빌드 번호를 부여합니다. 이미 승인된 버전으로는 새 빌드를 제출할 수 없으므로, 다음 배포 전 `project.yml`의 `MARKETING_VERSION`을 올리고 XcodeGen으로 프로젝트를 재생성합니다. 실제 배포 상태는 다운로드 안내와 App Store에서 확인하세요.
